...
namespace ROG_BackStage.DAL
{
    public class BaseDAL
    {
        public ValidationErrors validError = new ValidationErrors();
        /// <summary>
        /// 資料庫連線字串
        /// </summary>
        ...
        /// 資料庫連線設定
        /// </summary>
        protected int _connectionTimeout = 20;
        ...
        public Tuple<IEnumerable<T>, IEnumerable<U>, IEnumerable<E>> DapperThreeMultiQuery<T, U, E>(string querySql, object param = null, CommandType commandType = CommandType.Text)
        {
            using (IDbConnection con = GetDbConnection())
            using (var multi = con.QueryMultiple(querySql, param))
            {
                ...
                return new Tuple<IEnumerable<T>, IEnumerable<U>, IEnumerable<E>>(Multi_One, Multi_Two, Multi_Three);
            }
        }
        ...
        /// <typeparam name="T">更新資料物件Type</typeparam>
        /// <typeparam name="T1">新增資料物件Type</typeparam>
        /// <typeparam name="T2">新增資料物件Type</typeparam>
        /// <param name="updateEnties">更新物件</param>
        /// <param name="insertEnties1">新增物件</param>
        /// <param name="insertEnties2">新增物件</param>
        /// <returns>是否成功</returns>
        public bool DapperUpdateSingleInsertTwo<T, T1, T2>(T updateEnties, T1 insertEnties1, T2 insertEnties2)
            where T : class
            where T1 : class
            where T2 : class
        {
            ...
            using (var transaction = con.BeginTransaction(IsolationLevel.ReadUncommitted))
            {
                try
                {
                    if (con.Update(updateEnties, transaction, _connectionTimeout))
                    {
                        if (con.Insert(insertEnties1, transaction, _connectionTimeout) > 0)
                        {
                            if (con.Insert(insertEnties2, transaction, _connectionTimeout) > 0)
                            {
                                transaction.Commit();
                                isOk = true;
                            }
